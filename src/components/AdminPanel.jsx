...
const AdminPanel = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await fetch("http://localhost:3000/api/users"); // corrected
    ...
    const fetchVisitData = async () => {
      try {
        const response = await fetch(
          "http://localhost:3000/api/visit-count"
        );
        const data = await response.json();
  ...
  useEffect(() => {
    const getHerbs = async () => {
      try {
        const herbsArray = await fetchHerbs(); // directly the array
        setHerbs(herbsArray);
        setHerbCount(herbsArray.length); // ✅ works now
      } catch (error) {
  ...
  const handleDeleteHerb = async (herbId) => {
    ...
    try {
      await deleteHerb(herbId);
      alert("Herb deleted successfully!");
      setHerbs((prevHerbs) => prevHerbs.filter((herb) => herb.id !== herbId));
    } catch (error) {
      console.error("Error deleting herb:", error);
      alert("Failed to delete herb.");
    }
  ...
  //update herb data
  const handleUpdateHerb = async (herbId, updatedData) => {
    try {
      await updateHerb(herbId, updatedData);
      alert("Herb updated successfully!");
      
      setHerbs((prevHerbs) =>
        prevHerbs.map((herb) =>
          herb.id === herbId ? { ...herb, ...updatedData } : herb
        )
      );
    } catch (error) {
      console.error("Error updating herb:", error);
      alert("Failed to update herb.");
    }
  };
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      <a
                        ...
                        className="bg-blue-900 text-white px-4 py-2 rounded m-2"
                      >
                        View 3D Model
                      </a>
                      <button
                        onClick={() => handleDeleteHerb(herb.id)}
                        className="bg-red-600 text-white px-4 py-1 rounded m-2"
                      >
                        Delete
                      </button>
